...
const MODULE_MAP: Record<string, { route: string; icon: string }> = {
  grammar: { route: '/features/grammar-engine', icon: '📚' },
  speaking: { route: '/features/public-speaking', icon: '🎙️' },
  interview: { route: '/features/interview-training', icon: '💼' },
  pronunciation: { route: '/lessons/pronunciation', icon: '🔊' },
  vocabulary: { route: '/lessons/flashcards', icon: '🃏' },
  email: { route: '/features/email-writing', icon: '📧' },
  office: { route: '/features/office-conversations', icon: '🏢' },
  greetings: { route: '/features/daily-greetings', icon: '👋' },
  phone: { route: '/features/phone-simulator', icon: '📞' },
  quiz: { route: '/lessons/quiz', icon: '📝' },
}
...
export async function generateDailyFeed(userId: string): Promise<DailyFeed> {
  ...
  const greeting = hour < 12
    ...
    : hour < 17
    ? 'Good afternoon! Let\'s practice! ☀️'
    : 'Good evening! Evening learning is powerful! 🌙'
